...
async function key(p: Page, keyName: string, code = keyName, modifiers = 0): Promise<void> {
	...
	await p.send("Input.dispatchKeyEvent", { type: text ? "keyDown" : "rawKeyDown", key: keyName, code, modifiers, ...(text ? { text } : {}) });
...
try {
	...
	ok("header name is '<app title> <workspace title>'", name.text === "ERPNext Projects", JSON.stringify(name));
	ok("prefix span carries the app title", name.prefix === "ERPNext");
	ok("header name links to the sidebar's first link", !!name.href && name.href === name.firstSidebarHref, `${name.href} vs ${name.firstSidebarHref}`);
	...
	ok("nav labels equal the sidebar's top-level labels", JSON.stringify(nav.labels) === JSON.stringify(nav.sidebarLabels), JSON.stringify(nav));
	ok("every populated Section Break is a sub-menu", nav.submenus === nav.sections && nav.submenus >= 2, `${nav.submenus} vs ${nav.sections}`);
	...
	await sleep(250);
	const opened = await page.eval<{ expanded: string | null; display: string; width: string; bg: string; hash: string }>(`(() => {
	...
	ok("clicking a sub-menu title opens it (aria-expanded + CSS)", opened.expanded === "true" && opened.display === "flex", JSON.stringify(opened));
	ok("open sub-menu is 16rem wide on the g100 $layer", opened.width === "256px" && opened.bg === "rgb(38, 38, 38)", `${opened.width} ${opened.bg}`);
	...
	ok("Escape closes the sub-menu and returns focus to its title", afterEsc.expanded === "false" && afterEsc.focused, JSON.stringify(afterEsc));
	...
	ok("navigating to /desk/task marks the Task link current", current.current.includes("Task") && current.title === "Projects", JSON.stringify(current));
	...
	ok("a current child marks its collapsed sub-menu title", nested.child.includes("Activity Type") && nested.titles.includes("Setup"), JSON.stringify(nested));
	...
	await sleep(700);
	interface Overflow { more: boolean; hidden: number; inMore: number; overlap: boolean; total: number; name: string }
	const OVERFLOW = `(() => {
	...
	ok("13-row sidebar overflows into More at 1056px", overflow.more && overflow.hidden > 0 && overflow.total === 13, JSON.stringify(overflow));
	...
	ok("at 1600px more rows fit than at 1056px", wide.hidden < overflow.hidden && !wide.overlap, JSON.stringify(wide));
	...
	await sleep(500);
	const after = await page.eval<{ expanded: boolean; aria: string | null; glyph: boolean }>(`(() => ({ expanded: document.querySelector('.body-sidebar-container').classList.contains('expanded'), aria: document.querySelector('.cds--header__menu-toggle').getAttribute('aria-expanded'), glyph: !!document.querySelector('.cds--header__menu-toggle svg') }))()`);
	ok("hamburger toggles the sidebar and mirrors it in aria-expanded", before.expanded !== after.expanded && after.aria === String(after.expanded) && before.aria === String(before.expanded), JSON.stringify({ before, after }));
	ok("hamburger keeps the Menu glyph", after.glyph);
	await page.eval(`document.querySelector('.cds--header__menu-toggle').click()`);
	await sleep(400);

	// -- 8. bell + badge --------------------------------------------------------------
	await page.eval(`document.querySelector('.cf-shell-header .sidebar-notification .cds--header__action').click()`);
	await sleep(500);
	const bell = await page.eval<{ shown: boolean; position: string; top: string; width: string; bg: string }>(`(() => {
	...
	ok("bell opens the notifications as a fixed right panel under the header", bell.shown && bell.position === "fixed" && bell.top === "48px" && bell.width === "360px" && bell.bg === "rgb(38, 38, 38)", JSON.stringify(bell));
	await page.screenshot(SHOT + "/shell-notifications.png");
	await page.eval(`document.querySelector('#cf-switcher-button').click()`);
	await sleep(300);
	const yielded = await page.eval<boolean>(`document.querySelector('.cf-shell-header .dropdown-notifications').classList.contains('hidden')`);
	...
	ok("update_count_badge paints the header's badge after the move", badge.text === "3" && !badge.hidden && badge.bg === "rgb(250, 77, 86)", JSON.stringify(badge));
	...
	ok("header is 48px, fixed, z 1030 on g100 tokens (light theme)", light.height === "48px" && light.position === "fixed" && light.z === "1030" && light.bg === "rgb(22, 22, 22)" && light.border === "rgb(57, 57, 57)", JSON.stringify(light));
	ok("every utility is a 48x48 cell, no gaps, icon on $icon-secondary", light.cellCount === 4 && light.cells.every(([w, h, c]) => w === 48 && h === 48 && c === "rgb(198, 198, 198)") && light.gap === 0, JSON.stringify(light.cells));

	await page.eval(`document.documentElement.setAttribute('data-theme', 'dark')`);
	await sleep(600);
	const dark = await geometry();
	ok("header is identical in the dark theme", dark.bg === light.bg && dark.border === light.border && dark.cells[0]?.[2] === light.cells[0]?.[2], JSON.stringify(dark));
	...
	const narrow = await page.eval<string>(`getComputedStyle(document.querySelector('.cf-shell-header .cds--header__nav')).display`);
	ok("below lg (66rem) the header nav hides and the sidebar carries the links", narrow === "none", narrow);
	await viewport(page, 1600, 1000);

	// -- 10. landing page ---------------------------------------------------------
	await page.goto(`${BASE}/desk`);
	await page.waitFor(`!!document.querySelector('.cf-shell-header .cds--header__name') && !!document.querySelector('.desktop-container')`, { timeout: 90000 });
	await sleep(1500);
	const landing = await page.eval<{ name: string; navHidden: boolean; menuDisabled: boolean; searches: number; bells: number; navbars: number }>(`(() => {
		const h = document.querySelector('.cf-shell-header');
	...
	await sleep(2500);
	const mobile = await page.eval<{ shell: boolean; bodyClass: boolean; sticky: boolean }>(`(() => ({ shell: !!document.querySelector('.cf-shell-header'), bodyClass: document.body.classList.contains('cf-has-shell'), sticky: !!document.querySelector('.main-section .sticky-top') }))()`);
	ok("under 768px the shell does not mount and frappe's own header stands", !mobile.shell && !mobile.bodyClass && mobile.sticky, JSON.stringify(mobile));
