...
try {
  ...
  const rerender = await page.eval(`(() => {
    const l = cur_list;
    l.render_list();
    l.render_list();
  ...
  ok(
    ...
    align.metaTrailing && Math.abs(align.metaTrailing.th - align.metaTrailing.td) <= 1,
  ...
  const rv = await page.eval(`(() => ({ ctor: cur_list.datatable.constructor.name, rows: document.querySelectorAll('tbody .dt-row').length, view: cur_list.view_name }))()`);
