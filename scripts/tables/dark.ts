...
// `returnByValue` JSON the driver cannot check — so each probe names the shape
// its own browser-side expression builds. Every field here is a computed
// colour, i.e. whatever string `getComputedStyle` resolved to.
interface ListColours {
	th: string;
	td: string;
	bg: string;
}
interface ReportColours {
	th: string;
	tdBg: string;
	tdColor: string;
}
...
try {
	...
	const l = await page.eval<ListColours>(`(() => {
	...
	ok(
		"list view picks up g100 tokens",
		l.th !== "rgb(255, 255, 255)" && l.bg !== "rgb(255, 255, 255)",
		JSON.stringify(l),
	);
	...
	await page.eval(dark);
	await new Promise((r) => setTimeout(r, 700));
	const q = await page.eval<ReportColours>(`(() => {
    const th = document.querySelector('.cf-table thead th');
    const td = document.querySelector('.cf-table tbody td');
	...
	await page.waitFor(
		`!!(window.cur_frm && cur_frm.fields_dict && cur_frm.fields_dict.items && cur_frm.fields_dict.items.grid.carbon_table)`,
		{ timeout: 90000 },
	);
	await page.eval(
		`(() => { for (let i=0;i<3;i++) cur_frm.add_child('items',{qty:i+1,rate:(i+1)*100}); cur_frm.refresh_field('items'); return true; })()`,
	);
	...
	const g = await page.eval<GridColours>(`(() => {
