...
- **Bundle entry names**: the four `carbon_*.bundle.js` entry points must keep their `.js` names, even though most of the code behind them is TypeScript. frappe keys `assets.json` by the _entry_ basename (`esbuild.js:450`) but `hooks.py`/`include_script` look up the _output_ basename minus the hash (`esbuild.js:181-185`) with no extension fallback — so a `.ts` entry wrote a `carbon_desk.bundle.**ts**` key that nothing loads while the served `carbon_desk.bundle.**js**` key silently kept an older build's hash, serving a stale or swept bundle with no error. Naming the entries `.js` makes both keying paths agree in every build mode, `bench watch` included (fixed 2026-09-17; `carbon_charts` and `carbon_desk` keep their TS in sibling `carbon_charts.ts` / `carbon_desk.ts` behind import-only entries). No manual heal is needed after a watch session any more. `scripts/patch-assets.ts` and `carbon_frappe/build.py` still re-point the keys and `npm run audit` still checks them, but all three are now tripwires: if one ever reports a repair, an entry's name has drifted from its output again.
