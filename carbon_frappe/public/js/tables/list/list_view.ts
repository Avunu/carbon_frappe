 ...
 */
function seedColumnWidths(listview: ListView): void {
...
function buildColumns(listview: ListView): CarbonColumnSpec<FrappeListDoc>[] {
		...
		() =>
			function (this: ListView) {
				...
				for (const doc of this.data) {
					doc._idx = idx++;
					if (doc._assign) {
						// `_assign` is `json.dumps`'d server-side, so this is an
						// array of user ids; `JSON.parse` still hands back
						// `any`, and `Array.isArray` is the narrowing that
						// replaces the old `?.length || 0` — same 0 for every
						// non-array `_assign` frappe can produce (`"null"`, or
						// a parse that yields an object with no `length`).
						const assigned: unknown = JSON.parse(doc._assign);
