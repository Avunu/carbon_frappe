...
//
// ./expand declares the two engine members IT reaches (`instanceId`, `table`).
// This file reaches two more, so it extends that declaration rather than
// restating it — same discipline, one member set per module, and the real
// `CarbonTable` still has to satisfy every one of them.

/** The `grid.carbon_table` surface the detail panel drives. */
...
/** {@link CarbonGridHost} with the wider engine this file needs off it. */
export interface CarbonGridRowHost extends CarbonGridHost {
	carbon_table?: CarbonGridRowEngine | undefined;
}

/**
 * `wrapper` and `row` are declared {@link JQueryRegion} on frappe's GridRow —
 * a handle whose element 0 is guaranteed — because upstream builds both from a
 * literal template. This one is built from a literal template too, but jQuery's
 * own return type cannot say so, and the guarantee is exactly what the base
 * class's `.get(0)` overloads depend on. `length > 0` IS that guarantee, so
 * checking it is the honest way to hand it over.
 */
function isRegion($el: JQuery<HTMLElement>): $el is JQueryRegion {
	return $el.length > 0;
}

export default class CarbonGridRow extends GridRow implements CarbonGridRowShape {
	// ---------------------------------------------------- carbon's own members
	//
	// All `declare`: every one of these is assigned imperatively (some of them
	// from ./expand and ./row_menu, which write onto the row they are handed),
	// and GridRow's constructor calls `this.make()` before it returns —
	// grid_row.js:19. A real class field would be defined AFTER `super()`
	// finishes and would therefore blank whatever `make()` had already put
	// there. `declare` emits nothing, which is the runtime shape this class has
	// always had.

	/** Narrowed from `Grid`: a CarbonGridRow only ever belongs to a CarbonGrid. */
	declare grid: CarbonGridRowHost;

	/** The permanently-present child `<tr>`; owned by ./expand. */
	declare form_row?: HTMLTableRowElement | undefined;
	/** Its single `<td>`; owned by ./expand. */
	declare form_cell?: HTMLTableCellElement | undefined;
	/** `cds--child-row-inner-container` — the detail form's host; owned by ./expand. */
	declare form_inner?: HTMLDivElement | undefined;
	/** The chevron, built once and reused across renders; owned by ./expand. */
	declare expand_button?: HTMLButtonElement | undefined;
	/** The `⋮` trigger, built once and reused across renders; owned by ./row_menu. */
	declare row_menu_button?: HTMLButtonElement | undefined;

	/** The cell the `⋮` trigger lives in — frappe's trailing `.col`, or a fresh div. */
	declare menu_cell?: HTMLElement | undefined;
	/** The OUTER `.col` of frappe's open-form button, which frappe itself drops. */
	declare open_form_cell?: JQuery | undefined;
	/** "This call asked for the legacy dialog" — see {@link CarbonGridRow.toggle_view}. */
	declare _request_modal?: boolean | undefined;
	/** "The open form IS the legacy dialog" — latched in `show_form`, read in `hide_form`. */
	declare _modal_form?: boolean | undefined;
	...
	override make(): void {
		...
		const $row = $('<tr class="grid-row data-row"></tr>');
		if (!isRegion($row)) {
			throw new Error("CarbonGridRow.make: jQuery built an empty set from the row template");
		}
		this.wrapper = $row;
		this.row = this.wrapper;

		// Every branch returns explicitly: `return false` is jQuery's
		// "preventDefault and stopPropagation", and a handler that returns it on
		...
		const meta = this.grid.meta;
		if (this.grid.template && !meta) {
			throw new Error("CarbonGridRow.make: grid.template is set but grid.meta is missing");
		}

		let render_row: boolean | undefined = true;
		if (this.grid.template && meta && !meta.editable_grid) {
	 ...
	 * toolbar's magnifier owns that visibility now (tables/grid/toolbar.ts), and
	 ...
	 */
	override show_search_row(): boolean {
	...
	menu_node(): HTMLElement {
		if (!this.menu_cell) {
			// `.length` is precisely the emptiness test `.get(0)` needs, so the
			// throw below is unreachable — but it is the honest replacement for
			// the `undefined.classList` TypeError this used to raise if jQuery
			// ever disagreed with itself.
			const cell =
	 ...
	 */
	override add_open_form_button(): void {
