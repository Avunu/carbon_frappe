...
/**
 * The engine's profile seam, with `host` left as its `unknown` default: none of
 * the hooks below reads `ctx.host`, and a hook that accepts an unknown host
 * accepts every host, so this stays assignable wherever the engine wants a
 * profile pinned to its own type.
 */
export function gridProfile(): TableClassProfile {
	return {
		root(node) {
			add(node, "form-grid");
		},
...
/**
 * The slice of a cell context {@link gutter} reads.
 *
 * Spelled structurally, and with `column` optional, because the two hooks that
 * call it are handed different context types (a header cell's carries a
 * `header`, a body cell's a `row`) and because the runtime guard below predates
 * — and outlives — any promise the engine makes about which of them are filled.
 */
interface GutterContext {
	column?: TableClassColumn | undefined;
}
 ...
 */
function gutter(node: Element, ctx: GutterContext): void {
