...
//
// The inline fallbacks exist ONLY so the engine renders in scripts/dev-table.ts,
// which runs outside a desk and therefore has no `frappe` global. They are never
// reached in the app.

import type { FrappeIconSize } from "frappe-types";

/**
 * Keyed by sprite name, so an unknown name falls through to an empty glyph
 * rather than a type error: adapters pass names this table never enumerated
 * (tables/grid/toolbar.js builds its own), and on a desk the sprite sheet — not
...
const FALLBACK: Record<string, string> = {
	...
	"es-line-search":
		'<circle cx="7" cy="7" r="4" fill="none" stroke="currentColor"/><path d="M10 10l3 3" stroke="currentColor"/>',
...
export function icon(name: string, size: FrappeIconSize = "sm"): string {
